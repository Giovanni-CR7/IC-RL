...
if __name__ == "__main__":
    ...
    if mode == "train":
        ...
        model = PPO(
            ...
            learning_rate=3e-4,             
            gamma=0.999,                    # para foca no longo prazo (episódios longos) 
            ent_coef=0.01,                  # adiciona aleatoriedade para incentivar exploração 
            n_steps=1024,                   # coleta 8192 amostras antes de atualizar (8x1024) 
            batch_size=512,                 # quantidade de pacotes de dados passados na rede por vez 
            n_epochs=10,                    # revisões feitas nas amostras coletadas 
            vf_coef=0.5,                    # peso balanceado para a estimativa de pontuação 
            clip_range=0.2,                 # limita o salto de aprendizado a 20% 
            max_grad_norm=0.5,              # corta picos matemáticos para evitar desaprender
            target_kl=0.02,                 # interrompe o "lote" se a divergência passar de 0.02 
